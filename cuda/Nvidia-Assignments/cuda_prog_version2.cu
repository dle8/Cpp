...
__global__ void kernel_A(float *g_data, int dimx, int dimy, int niterations) {
    // Add stride registers to reduce extra computation in for loops predicate
    int stride_x = blockDim.x * gridDim.x;
    int stride_y = blockDim.y * gridDim.y;

    for (int iy = blockIdx.y * blockDim.y + threadIdx.y; iy < dimy; iy += stride_y) {
        for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < dimx; ix += stride_x) {
            int idx = iy * dimx + ix;

            float value = g_data[idx];

            // Move iteration inside if predicate so that each module only run max 4 times instead of max 4 * niterations time
            if (ix % 4 == 0) {
                for (int i = 0; i < niterations; ++i)
                    value += sqrtf(logf(value) + 1.f);
            } else if (ix % 4 == 1) {
